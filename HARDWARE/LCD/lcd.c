...
void LCD_Gui_Drawbmp16(u16 x,u16 y,u16 weith,u16 height,const unsigned char *p) //显示40*40 QQ图片
{
  	u8 i,j; 
	LCD_Set_Window(x,y,x+weith-1,y+height-1);//窗口设置
    for(i=0;i<height;i++)
	{	
		for(j=0;j<weith;j++)			
		{
			LCD_Fast_DrawPoint(y+j,x+i,p[i*weith+j]);  		
		}
	}	
